...
### 🔔 访客记录中心 · 铃铛通知(最新 · 第十七波)
用户需求:「记录访问平台的用户 —— 哪个 IP 过来、访问了哪些页面、看了什么、做了哪些操作;在右上角铃铛里点一下就能查看,每个独立 IP 形成一条独立记录」。完整落地:
- **🗄️ 持久化埋点 · SQLite `visit_logs` 表**:每条 = 一次页面浏览(`page`)或操作(`action`),记录 `ip / page / title / detail / company / role / ua / created_at`。`db.log_visit()` 落库,`db.visit_sessions()` **按独立 IP 聚合**成会话(首次/最近访问时间、访问次数、去重页面数、涉及公司/角色、设备类型 + 明细事件列表)。
- **🌐 真实 IP 识别**:后端 `_client_ip()` 优先取 `cf-connecting-ip` / `x-forwarded-for` / `x-real-ip`(兼容 Cloudflare/Nginx 反代),回退直连 IP —— 上线后能拿到访客真实公网 IP。
- **📍 双通道埋点**:① 后端 `GET /` 自动记录"进入平台";② 前端 `trackVisit()` 上报到 `POST /api/track` —— **包装 `go()` 路由自动记录页面浏览**,并在切换公司/切换角色/保存配置/AI 提问等关键动作记录操作。页面名经 `navTitle()` 翻译为人类可读双语名(工作台/全球合规中心/AI 老板驾驶舱…)。
- **🔔 铃铛通知中心**:右上角铃铛红点显示**独立 IP 数**(每分钟自动刷新);点击弹出访客记录面板 —— **每个独立 IP 一张可折叠卡片**(IP + 国旗 + 设备 + 访问次数/页面数 + 相对时间),展开看该 IP 的完整行为流水(浏览👁/操作👆 图标 + 标题 + 细节 + "几分钟前")。
- **i18n 双语 + 相对时间**:新增 `visit` 文案块(中英),`_ago()` 把 UTC 时间戳转"刚刚/N分钟前/N小时前/N天前"。
- Playwright E2E **13 项断言全过 · 0 JS 错误**:铃铛存在、红点计数、后端记录独立 IP/多条访问、page+action 双类型、含"切换公司"操作 + 真实页面名、点铃铛弹窗按 IP 分组展示行为流水。

### 🗺️ 打卡地点接入真实地图 · Leaflet + OpenStreetMap(第十六波)
用户问「打卡地点这里是否可以调用真实地图显示出坐标」。诊断发现旧版是**纯 CSS 模拟的假地图**(拖图钉时坐标用 `lat = 3.20 - y*0.12` 在框里"假算"出来,根本不知道图钉在地球上哪),遂换成**真实地图**:
- **🌍 真实地图引擎 · Leaflet 1.9.4 + OpenStreetMap**:免费、免 API Key、全球覆盖。CDN 直引,无需后端代理。真实可缩放/平移的世界地图瓦片。
- **📍 按公司所在国自动定位中心**:后端新增 `_ATTEND_GEO`(7 国真实总部城市坐标:吉隆坡/新加坡/曼谷/胡志明/雅加达/香港/上海),`attendance_loc` 模块按公司 `country_code` 透传 `geo{lat,lng,zoom}` + `coord` + 本地化地址/邮编。实测切中国→地图落点上海(31.23,121.47)、切新加坡→莱佛士坊(1.28,103.85)、切马来→吉隆坡(3.15,101.71)。
- **🖱️ 可拖动图钉 + 点击落点 + 坐标实时回填**:`L.marker({draggable})` 拖动 / 地图点击 → 经纬度精确到 6 位小数自动写回「坐标」框;半径圈 `L.circle({radius:米})` 随有效打卡半径输入框联动缩放。
- **🔎 地址搜索定位 · OSM Nominatim**:搜索框输入地址/地点(按公司国别 `countrycodes` 缩小范围)→ 免费地理编码 API 返回坐标,地图飞过去 + 图钉/圈/坐标同步。实测搜 "Petronas Twin Towers" → 3.157968, 101.711205(双子塔毫米级精准)。
- **i18n 双语**:新增 `map` 文案块(坐标/拖动提示/半径/搜索/定位/未找到),中英文齐备。
- Playwright E2E **15 项断言全过 · 0 JS 错误**:CN/SG/MY 三国真实瓦片加载(6/9/4 张)、容器渲染、图钉存在、中心精准、点击回填、地址搜索全部通过。

### 🔗 数据流一致性体检与修复 · 全链路同源(第十五波)
...
- **🪙 收尾 · 发薪表币种前缀**:公式发薪表格金额改为「**币种前缀 + 数字**」(CN `¥ 35,000.00` · HK `HK$ 52,000.00` · TH `฿ 85,000.00` · MY `RM 9,500.00`),`/api/payroll/run` 顶层返回 `currency`/`currency_prefix`,合计行标注币种。同时**合并 `fmtMoney` 重复定义**(此前 app.js 中定义了两次,后者覆盖前者 —— 同「重复路由」类隐患),全站金额格式统一。E2E 验证 4 国前缀正确 + 本地化名一致 + 0 JS 错误。
